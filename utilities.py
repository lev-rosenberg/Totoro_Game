...
def make_car1(canvas, size, top_left=(0, 0), color="#3D9970", tag=None):
    '''
    demo function that show you how to draw a car, given the convenience
    functions that are available in this module
    '''
    x, y = top_left
    make_rectangle(canvas, (x + size*2, y + size*0.03), size*0.4, size*0.4, color=color, tag=tag)
...
def make_image(
    ...
    dir_path = os.path.dirname(os.path.realpath('Toon_Road_Texture2.png'))
    image_path = os.path.join(dir_path, image_path)
    pil_image = Image.open(image_path)
    if scale:
        size = (
            round(pil_image.size[0] * 1.3*scale), 
